...
def gur()->Tuple[int,int]:
    ...
    end_greater_than_start_validator=lambda n:"The end of the range must be greater than " \
                                                 "or equal to the start of the range." if int(n)<s else None
    
    s=int(prompt_until_predicate_met("Please enter the start of the range of integers you want to choose from: ",
        integer_validator))
    print()
    
    end=int(prompt_until_predicate_met("Please enter the end of the range of integers you want to choose from: ",lambda n:integer_validator(n)or end_greater_than_start_validator(n)))
    print()
    
    return s,end
def binary_search_guess_number(s:int,end:int)->Optional[Tuple[int,int]]:
    """
    ...
    :param s: The start of the range of integers
    ...
    while(s<=end):
        ...
        guess_comparison=prompt_until_predicate_met(f"Is your number {mid}? Enter 'h' if your number is higher, 'l' if your number is lower, or 'c' if my guess is correct: ",lambda comparison:"Please enter 'h', 'l', or 'c'." if comparison not in {"h","l","c"} else None)
...
def main()->None:
    ...
    print(f"I will guess the number you are thinking of in {num_tries} tries or less.\n")
    
    result=binary_search_guess_number(start,end)
    print()
    
    if(result is None):
        t("You are a dirty cheater, aren't you? You must not even have had a number in mind.")
    else:
        t(f"I guessed your number {result[0]} in {result[1]}/{num_tries} guesses!")
